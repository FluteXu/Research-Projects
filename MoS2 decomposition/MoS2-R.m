...
for i=1:Simnum
    ...
    tY=ceil(rand(1,1)*l);
    tZ=ceil(rand(1,1)*3);
    while (all(ismember(temX,tX))==1 && all(ismember(temY,Y))==1 && all(ismember(temZ,Z))==1) || tZ==1 && tY>l-tX+1 || tZ==2 && tY>=l-tX+1 || tZ==3 && tY>=l-tX+1    % not repeat & in the triangle
            tX=ceil(rand(1,1)*l);
            tY=ceil(rand(1,1)*l);
            tZ=ceil(rand(1,1)*3);
    end
    if tZ==1 && rand(1,1)>=0.3 %add PMo=0.9 condition
