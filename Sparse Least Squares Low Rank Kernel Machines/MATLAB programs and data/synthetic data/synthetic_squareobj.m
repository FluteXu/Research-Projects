...
for Iter=1:100
    for j=1:M;
        ...
        for i=1:m
            tempKC = Phi(:,j)*tempC(:,i)'+tempC(:,i)* Phi(:,j)';
            tempKMu = Phi(:,j)*tempMu(:,i)'+tempMu(:,i)*Phi(:,j)';
            % 2nd term
            ParE1(i)=  trace(((t.* theta(2:N+1))'*Phi(:,j)) * Phi(:,j)' * (tempKC*((t.* theta(2:N+1))))  + ...
                ((t.* theta(2:N+1))'*tempKC) * Phi(:,j) * (Phi(:,j)'*(t.* theta(2:N+1))))  ...
                +2*theta(1)*ones(N,1)'*tempKC*(t.* theta(2:N+1));
            ParE2(i)=  trace(((t.* theta(2:N+1))'*Phi(:,j)) * Phi(:,j)' * (tempKMu*((t.* theta(2:N+1))))  + ...
                ((t.* theta(2:N+1))'*tempKMu) * Phi(:,j) * (Phi(:,j)'*(t.* theta(2:N+1)))) ...
                +2*theta(1)*ones(N,1)'*tempKMu*(t.* theta(2:N+1));
        end;
