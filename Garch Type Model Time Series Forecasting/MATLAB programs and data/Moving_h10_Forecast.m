...
mmm_return_forecast_2=zeros(length(mmm_rtn_os),1); 
mmm_return_forecast_3=zeros(length(mmm_rtn_os),1); 
% HS25 - Return
window = 25;
weights = repelem(1/window, window);
...
% HS100 Variance
mmm_vol_forecast_3=zeros(length(mmm_rtn_os),1);
% HS25 Variance
...
for i = 1:21
    ...
    coke_return_sample = coke_rtn(1:end-840+10*(i-1));
    
    %%%%%%%%%%%%%%%%%%%%%%% 1. Coke AR(1)-GARCH(1,1)-t
    Mdl = arima(1,0,0); Mdl.Variance=garch(1,1); Mdl.Distribution='T';  % refit the oject everytime
    [EstMdl,~,~,~] = estimate(Mdl,coke_return_sample,'display','off'); 
    %%% forecast return & volatility
    [coke_return_forecast_1(10*i-9:10*i),~,coke_vol_forecast_1(10*i-9:10*i)]=forecast(EstMdl,10,'Y0',coke_return_sample);
    
    %%%%%%%%%%%%%%%%%%%% 2. Coke AR(1)-GJR-GARCH(1,1)-t
    Mdl = arima(1,0,0); Mdl.Variance=gjr(1,1); Mdl.Distribution='T'; 
    [EstMdl,~,~,~] = estimate(Mdl,coke_return_sample,'display','off');
    %%% forecast return & volatility
    [coke_return_forecast_2(10*i-9:10*i),~,coke_vol_forecast_2(10*i-9:10*i)]=forecast(EstMdl,10,'Y0',coke_return_sample);

    %%%%%%%%%%%%%%%%%%%%%%%%%%% 3. I-GARCH(1,1)-N (For volatility only)
    [PARAMETERS,~,HT,~,~,~,~]=igarch(coke_return_sample,1,1,[],[],0);
    %%% forecast volatility
    coke_vol_forecast_3(10*i-9)=PARAMETERS*coke_return_sample(end)^2+(1-PARAMETERS)*HT(end);
    for t=2:10
        coke_vol_forecast_3(10*(i-1)+t)=HT(end)*(t-1);
    end       
    %%%%%%%%%%%%%%%%%%%%%%%%%%%% MSFT %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    msft_return_sample = msft_rtn(1:end-840+10*(i-1));
    %%%%%%%%%%%%%%%%%%%%%%%%% 1. MSFT AR(1)-GARCH(1,1)-t
    Mdl = arima(1,0,0); Mdl.Variance=garch(1,1); Mdl.Distribution='T'; 
    [EstMdl,~,~,~] = estimate(Mdl,msft_return_sample,'display','off');

    %%% forecast return volatility
    [msft_return_forecast_1(10*i-9:10*i),~,msft_vol_forecast_1(10*i-9:10*i)]=forecast(EstMdl,10,'Y0',msft_return_sample);
    
    %%%%%%%%%%%%% 2. MSFT AR(1)-GJR-GARCH(1,1)-t
    Mdl = arima(1,0,0); Mdl.Variance=gjr(1,1); Mdl.Distribution='T'; 
    [EstMdl,~,~,~] = estimate(Mdl,msft_return_sample,'display','off');
    %%% forecast return volatility
    [msft_return_forecast_2(10*i-9:10*i),~,msft_vol_forecast_2(10*i-9:10*i)]=forecast(EstMdl,10,'Y0',msft_return_sample);

    %%%%%%%%%%%%%% 3. MSFT-IGARCH-N (for risk only)
    [PARAMETERS,~,HT,~,~,~,~]=igarch(msft_return_sample,1,1,[],[],0);
    
    %%% forecast volatility
    msft_vol_forecast_3(10*i-9)=PARAMETERS*msft_return_sample(end)^2+(1-PARAMETERS)*HT(end);
    for t=2:10
        msft_vol_forecast_3(10*(i-1)+t)=HT(end)*(t-1);
    end 

    %%%%%%%%%%%%%%%%%%%%%%%%%%%% JNJ %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    jnj_return_sample = jnj_rtn(1:end-840+10*(i-1));
    %%%%%%%%%%%%%%%%%1.  JNP AR-GARCH-t
    Mdl = arima(1,0,0); Mdl.Variance=garch(1,1); Mdl.Distribution='T'; 
    [EstMdl,~,~,~] = estimate(Mdl,jnj_return_sample,'display','off');
    %%% forecast return volatility
    [jnj_return_forecast_1(10*i-9:10*i),~,jnj_vol_forecast_1(10*i-9:10*i)]=forecast(EstMdl,10,'Y0',jnj_return_sample);

    %%%%%%%%%%%%%%%%%2. JNP AR-GJR-GARCH-t
    Mdl = arima(1,0,0); Mdl.Variance=gjr(1,1); Mdl.Distribution='T'; 
    [EstMdl,~,~,~] = estimate(Mdl,jnj_return_sample,'display','off');
    %%% forecast return volatility
    [jnj_return_forecast_2(10*i-9:10*i),~,jnj_vol_forecast_2(10*i-9:10*i)]=forecast(EstMdl,10,'Y0',jnj_return_sample);
    
    %%%%%%%%%%%%%%%%%3. JNJ-IGARCH-N (for risk only)
    [PARAMETERS,~,HT,~,~,~,~]=igarch(jnj_return_sample,1,1,[],[],0);
    %%% forecast volatility
    jnj_vol_forecast_3(10*i-9)=PARAMETERS*jnj_return_sample(end)^2+(1-PARAMETERS)*HT(end);
    for t=2:10
        jnj_vol_forecast_3(10*(i-1)+t)=HT(end)*(t-1);
    end   
    %%%%%%%%%%%%%%%%%%%%%%%%%%%% MMM %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    mmm_return_sample = mmm_rtn(1:end-840+10*(i-1));
    %%%%%%%%%%%%%%%%%%%% 1. MMM-GJR-GARCH-t MMM
     Mdl = arima(1,0,0); Mdl.Variance=garch(1,1); Mdl.Distribution='T'; 
    [EstMdl,~,~,~] = estimate(Mdl,mmm_return_sample,'display','off');
    % forecast return & volatility
    [mmm_return_forecast_1(10*i-9:10*i),~,mmm_vol_forecast_1(10*i-9:10*i)]=forecast(EstMdl,10,'Y0',mmm_return_sample);
    %%%%%%%%%%%%%%%%%%%% 2. MMM - IGARCH
    [PARAMETERS,~,HT,~,~,~,~]=igarch(mmm_return_sample,1,1,[],[],0);
    %%% forecast volatility
    mmm_vol_forecast_2(10*i-9)=PARAMETERS*mmm_return_sample(end)^2+(1-PARAMETERS)*HT(end);
    for t=2:10
        mmm_vol_forecast_2(10*(i-1)+t)=HT(end)*(t-1);
    end 
    %%%%%%%%%%%%%%%%%%%%%%%%%%%% PTR %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ptr_return_sample = ptr_rtn(1:end-840+10*(i-1));
    %%%%%%%%%%%%%%%%%%%%%% 1. PTR ARMA(2,2)-GARCH(2,2)-T
    Mdl = arima(2,0,2); Mdl.Variance=garch(2,2); Mdl.Distribution='T';
    [EstMdl,~,~,~] = estimate(Mdl,ptr_return_sample,'display','off');
    %%% forecast return volatility
    [ptr_return_forecast_1(10*i-9:10*i),~,ptr_vol_forecast_1(10*i-9:10*i)]=forecast(EstMdl,1,'Y0',ptr_return_sample);
    ...
    %%% forecast return volatility
    [ptr_return_forecast_2(10*i-9:10*i),~,ptr_vol_forecast_2(10*i-9:10*i)]=forecast(EstMdl,10,'Y0',ptr_return_sample);
    
    %%%%%%%%%%%%%%%%%%%%%%%3. PTR IGARCH(1,1)-N (for risk only)
    [PARAMETERS,~,HT,~,~,~,~]=igarch(ptr_return_sample,1,1,[],[],0);
    ptr_vol_forecast_3(10*i-9)=PARAMETERS*ptr_return_sample(end)^2+(1-PARAMETERS)*HT(end);
    for t=2:10
        ptr_vol_forecast_3(10*(i-1)+t)=HT(end)*(t-1);
    end  
end
